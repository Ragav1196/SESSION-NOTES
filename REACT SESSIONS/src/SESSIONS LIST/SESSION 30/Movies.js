...
export function Movies({ name, poster, rating, summary, id, deleteButton, editButton }) {  
  const styles = {
  ...
  // USE HISTORY
    const history = useHistory();
      // history gets the return value of "useHistory()"
  ...
  return (
    <Card className="movieContainer">
      ...
      <div className="movieDetails">
        ...
        <IconButton aria-label="trailer" onClick={() => {
          history.push("/Movies/" + id)}}>
            {/* this method changes the url that specified in it */}
            {/* Then based on what content is given on that specified url in the route, will then be rendered on the screen */}

          <PlayArrowIcon fontSize="inherit" />
        </IconButton>

        <Typography variant="body2" color="text.secondary" className="movieSummary font" style={summaryStyles}>        
            {summary}       
        </Typography>
      </div>
      <ButtonForMovies />
